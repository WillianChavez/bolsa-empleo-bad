  ...
  React.useEffect(() => {
    const loadJobs = async () => {
      try {
        ...
        setError("Error al cargar las ofertas de trabajo. Intenta de nuevo más tarde.");
        console.error(err);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight md:text-3xl">Gestionar Ofertas de Trabajo</h1>
          <p className="text-sm text-muted-foreground">
            Aquí puedes ver, crear, editar y eliminar ofertas de trabajo.
          </p>
        </div>
        <Button
          onClick={handleNavigateToNewOffer}
        >
          {/* <PlusCircle className="mr-2 h-4 w-4" /> */}{/* Temporarily commented out */}
          Crear Nueva Oferta
        </Button>
      </div>
      ...
      {error && (
        <div className="rounded-md border bg-destructive/10 p-4 text-center text-destructive">
          <p>{error}</p>
        </div>
      )}
