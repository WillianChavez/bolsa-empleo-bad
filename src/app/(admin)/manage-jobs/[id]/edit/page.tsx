...
export default function EditJobOfferPage() {
    ...
    const loadJobOffer = async () => {
      ...
      try {
        ...
        toast.error('Error al cargar la oferta', {
          description:
            'No se pudieron cargar los datos de la oferta para editar.',
        })
      }
      setIsLoadingData(false)
    }
    ...
    return (
      <div className='space-y-6'>
        ...
        <p className='text-muted-foreground'>
          La oferta de trabajo que intentas editar no existe o no pudo ser
          cargada.
        </p>
      </div>
    )
  ...
  const initialFormValues: Partial<JobOfferFormValues> = {
    ...
    rango_salarial:
      jobOffer.rango_salarial === null || jobOffer.rango_salarial === undefined
        ? ''
        : jobOffer.rango_salarial,
    fecha_limite_aplicacion: jobOffer.fecha_limite_aplicacion
      ? new Date(jobOffer.fecha_limite_aplicacion)
      : undefined,
    ...
    <div className='space-y-6'>
      <div>
        ...
        <h1 className='text-2xl font-semibold tracking-tight md:text-3xl'>
          ...
          <span className='text-blue-600'>{jobOffer.nombre_puesto}</span>
        </h1>
      ...
      <JobOfferForm
        ...
        submitButtonText={
          isSubmitting ? 'Guardando Cambios...' : 'Actualizar Oferta'
        }
      />
    </div>
  )
