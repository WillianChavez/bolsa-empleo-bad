...
export const JobOffersDataTable: React.FC<JobOffersDataTableProps> = ({ data }) => {
  const router = useRouter();

  if (!data || data.length === 0) {
    return (
      <div className="rounded-md border p-10 text-center">
        <p className="text-muted-foreground">No hay ofertas de trabajo para mostrar.</p>
      </div>
    );
  }
                ...
                <TableCell className="text-right">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem onClick={() => handleViewDetails(offer.id_puesto)}>
                        <Eye className="mr-2 h-4 w-4" />
                        Ver Detalles
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => handleEdit(offer.id_puesto)}>
