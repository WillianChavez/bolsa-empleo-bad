...
export default function HomePage() {
  ...
  const [searchCriteria, setSearchCriteria] =
    useState<JobSearchCriteria | null>(null)
    ...
    <>
      <header className='mb-8 text-center'>
        ...
        <p className='mx-auto mt-3 max-w-md text-base text-gray-500 sm:text-lg md:mt-5 md:max-w-3xl md:text-xl'>
          Encuentra tu próxima oportunidad profesional. Filtra por palabra
          clave, salario y modalidad.
        </p>
      </header>
      ...
      {searchCriteria && (
        <div className='mb-6 rounded-lg bg-gray-100 p-4 text-center shadow'>
          ...
          <p className='text-sm text-gray-700'>
            Término: {searchCriteria.searchTerm || 'N/A'} | Salario Mín:{' '}
            {searchCriteria.minSalary || 'N/A'} | Salario Máx:{' '}
            {searchCriteria.maxSalary || 'N/A'} | Modalidad:{' '}
            {searchCriteria.modality || 'N/A'}
          </p>
        </div>
      )}
