...
import { ArrowLeft, Briefcase, Building, DollarSign, MapPin, CheckCircle, Info, Sparkles } from 'lucide-react';
import { toast } from 'sonner'; // Import toast from sonner

export default function JobDetailPage() {
  const params = useParams();
  const jobId = params.id ? parseInt(params.id as string, 10) : null;
  const [jobOffer, setJobOffer] = useState<JobOfferViewModel | null | undefined>(undefined); // undefined for loading, null for not found
  ...
  const handleApply = () => {
    ...
    setIsApplied(true);
    toast.success(`¡Felicidades! Has aplicado exitosamente a ${jobOffer?.nombre_puesto}`, {
      description: "Recibirás una notificación si la empresa avanza con tu postulación. ¡Mucha suerte!",
      ...
      <div className="container mx-auto px-4 py-8">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="h-10 bg-gray-200 rounded w-1/4 animate-pulse mt-6"></div>
          </CardContent>
        </Card>
      </div>
    );
  ...
  if (!jobOffer) {
    return (
        <div className="container mx-auto px-4 py-8 text-center">
            ...
            <h1 className="text-2xl font-semibold mb-2">Oferta no encontrada</h1>
            <p className="text-gray-600 mb-6">Lo sentimos, la oferta de trabajo que buscas no existe o ha sido eliminada.</p>
            <Button asChild>
      ...
      <Card className="overflow-hidden shadow-lg">
        <CardHeader className="bg-gray-50 p-6 border-b">
          <div className="flex flex-col sm:flex-row justify-between items-start">
            <div>
                <CardTitle className="text-3xl font-bold text-blue-700 mb-1">{jobOffer.nombre_puesto}</CardTitle>
                <CardDescription className="text-lg text-gray-600 flex items-center">
                    <Building className="mr-2 h-5 w-5 text-gray-500" /> {jobOffer.empresa.nombre_empresa}
                </CardDescription>
            </div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <Badge variant="secondary" className="text-sm py-1 px-3"><MapPin className="mr-1.5 h-4 w-4" /> {jobOffer.ubicacion}</Badge>
            <Badge variant="secondary" className="text-sm py-1 px-3"><Briefcase className="mr-1.5 h-4 w-4" /> {jobOffer.modalidad.nombre_modalidad}</Badge>
            {jobOffer.rango_salarial && (
              <Badge variant="secondary" className="text-sm py-1 px-3"><DollarSign className="mr-1.5 h-4 w-4" /> {jobOffer.rango_salarial}</Badge>
            )}
          </div>
        </CardHeader>
        ...
        <CardContent className="p-6 space-y-6">
          <div>
            <h2 className="text-xl font-semibold text-gray-800 mb-3">Descripción del Puesto</h2>
            <p className="text-gray-700 leading-relaxed whitespace-pre-line">
              {jobOffer.descripcion_corta} {/* TODO: Use full description from a more detailed job type if available */}
            </p>
          </div>
          ...
          {renderListItems((jobOffer as any).descripcion || jobOffer.descripcion_corta, "Descripción Completa")}
          {renderListItems((jobOffer as any).requisitos, "Requisitos")}
          {renderListItems((jobOffer as any).conocimientos_requeridos, "Conocimientos Requeridos")}
          {renderListItems((jobOffer as any).perfil_academico, "Perfil Académico")}
          {renderListItems((jobOffer as any).habilidades_requeridas, "Habilidades Requeridas")}
          {renderListItems((jobOffer as any).experiencia_requerida, "Experiencia Requerida")}
          
          <div className="pt-4 border-t mt-6">
            <Button 
              size="lg" 
              className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white text-lg disabled:opacity-70 disabled:cursor-not-allowed"
              onClick={handleApply}
