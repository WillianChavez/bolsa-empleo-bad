...
export const JobOfferList: React.FC<JobOfferListProps> = ({
  ...
  const filteredJobOffers = useMemo(() => {
    ...
    if (searchCriteria.modality && searchCriteria.modality !== 'all') {
      offers = offers.filter(
        (job) => job.modalidad.nombre_modalidad === searchCriteria.modality
      )
    }

    // Filter by salary range
    const searchMinSalary = searchCriteria.minSalary
      ? parseFloat(searchCriteria.minSalary)
      : null
    const searchMaxSalary = searchCriteria.maxSalary
      ? parseFloat(searchCriteria.maxSalary)
      : null
    ...
    if (searchMinSalary !== null || searchMaxSalary !== null) {
      offers = offers.filter((job) => {
        ...
        if (searchMinSalary !== null && searchMaxSalary !== null) {
          ...
          return (
            jobSalaryRange.min <= searchMaxSalary &&
            jobSalaryRange.max >= searchMinSalary
          )
        }
  ...
  if (isLoading) {
    return (
      <div className='grid grid-cols-1 gap-6 p-4 md:grid-cols-2 md:p-6 lg:grid-cols-3'>
  ...
  if (error) {
    return (
      <div
        role='alert'
        className='my-10 rounded-md border border-red-200 bg-red-50 p-4 text-center text-red-600'
      >
        Error: {error}
      </div>
    )
  ...
  if (filteredJobOffers.length === 0) {
    return (
      <div className='py-10 text-center text-gray-500'>
        No se encontraron ofertas de trabajo que coincidan con sus criterios de
        búsqueda.
      </div>
    )
  }
